...
export const ArticlesManager = () => {
  ...
  // Publishing only writes the row. The static prerender (and with it the
  // "Sourced from" attribution and JSON-LD) is regenerated by a Netlify build,
  // so the publish flow triggers one unless the operator opts out.
  const [deployAfterPublish, setDeployAfterPublish] = useState(true);
  const [deploying, setDeploying] = useState(false);

  const triggerProductionDeploy = async () => {
    setDeploying(true);
    const { data, error } = await supabase.functions.invoke("netlify-deploy", {
      body: { action: "deploy", target: "production" },
    });
    setDeploying(false);
    if (error) {
      toast.error("Published, but the production deploy could not be triggered.", {
        description: error.message,
      });
      return;
    }
    if (data?.unavailable) {
      toast.warning("Published, but no Netlify build hook is configured.", {
        description: "Set NETLIFY_BUILD_HOOK_URL, then deploy from the Bundles tab.",
      });
      return;
    }
    toast.success("Production deploy triggered.", {
      description: "Prerendered attribution goes live when the Netlify build finishes.",
    });
  };
  ...
  const save = async (skipPreview = false) => {
    ...
    setEditorOpen(false);
    load();
    if (draft.is_published && deployAfterPublish) await triggerProductionDeploy();
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-start gap-3">
              <Switch
                id="deploy-after"
                checked={deployAfterPublish}
                disabled={!draft.is_published}
                onCheckedChange={setDeployAfterPublish}
              />
              <div>
                <Label htmlFor="deploy-after">Deploy to production after publishing</Label>
                <p className="text-xs text-muted-foreground">
                  Runs a Netlify production build so the prerendered page, its "Sourced from"
                  attribution and JSON-LD go live. Without a build, only the app shell updates.
                </p>
              </div>
            </div>
